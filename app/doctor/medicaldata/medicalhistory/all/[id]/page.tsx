...
type MedicalRecordData = {
  ...
  Assessment: string;
  TreatmentPlan: TreatmentPlan | null; // Updated Treatment type to TreatmentPlan
  createdAt?: string;
  updatedAt?: string;
...
export default function MedicalFindingForm({ params }: MedicalFindingFormProps) {
  ...
  const renderTreatment = (treatment: TreatmentPlan | null) => {
    ...
    // Debugging log to see the true treatments being returned
    console.log("True Treatments:", trueTreatments);
    ...
    toast.warn(
      <div>
        ...
        <div style={{ display: "flex", justifyContent: "space-between", marginTop: "10px" }}>
          ...
          >
            Yes</button>
          <button onClick={() => toast.dismiss()} className=" text-black px-2 py-1 rounded-md">
            No
          </button>
        </div>
      </div>,
    ...
    return (
        <div key={finding._id} className="border p-4 rounded-lg shadow-md flex items-start justify-between">
            ...
            <div className="flex-grow px-4">
                ...
                <p><strong>Treatment:</strong> {renderTreatment(finding.TreatmentPlan)}</p>
            </div>
            <div className="flex flex-col space-y-2">
                <button
