    ...
    const confirmDelete = async () => {
      try {
        ...
        if (response.data.success) {
          ...
          toast.error(response.data.error || "Error deleting Appointment");
        }
      } catch (error) {
    ...
    <div>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", marginTop: "10px" }}>
        <button
          ...
          className=" text-red px-1 py-1 rounded-md"
        >
          Yes</button>
        <button onClick={() => toast.dismiss()} className=" text-black px-1 py-1 rounded-md">
          No
        </button>
      </div>
    </div>,
