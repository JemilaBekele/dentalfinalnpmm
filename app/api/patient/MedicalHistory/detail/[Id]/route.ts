...
export async function GET(request: NextRequest, { params }: { params: { patientId: string; recordId: string } }) {
    ...
    try {
      ...
      const finding = await MedicalFinding.findById(recordId).exec();
      if (!finding) {
  ...
  export async function PATCH(request: NextRequest) {
    ...
    try {
      ...
      const updatedFinding = await MedicalFinding.findByIdAndUpdate(recordId, data, { new: true }).exec();
      if (!updatedFinding) {
        return NextResponse.json({ error: "Medical finding not found" }, { status: 404 });
      }
  ...
  export async function DELETE(request: NextRequest) {
    ...
    try {
      ...
      const deletedFinding = await MedicalFinding.findByIdAndDelete(recordId).exec();
      if (!deletedFinding) {
        return NextResponse.json({ error: "Medical finding not found" }, { status: 404 });
      }
