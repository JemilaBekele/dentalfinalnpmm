...
export async function GET(request: NextRequest, { params }: { params: { id: string } }) {
    ...
    try {
        ...
        // Find the patient by ID and populate the Image field
        const patient = await Patient.findById(id).populate('Image').exec();
        if (!patient) {
            return NextResponse.json({ error: "Patient not found" }, { status: 404 });
        }
        ...
        if (!patient.Image || patient.Image.length === 0) {
            return NextResponse.json({ message: "No images available for this patient", data: [] });
        }
        const sortedFindingsImage = patient.Image.sort((a: Image, b: Image) => {
            return new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime();
          });
        // Return the populated images associated with the patient
        return NextResponse.json({
            message: "Images retrieved successfully",
            success: true,
            data: sortedFindingsImage,
        });
    } catch (error) {
